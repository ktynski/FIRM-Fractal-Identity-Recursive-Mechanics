...
class EffectiveNeutrinoSpecies:
    ...
    def derive_neff_formula(self, method: ChannelMethod = ChannelMethod.COMBINED_METHOD) -> Dict[str, Any]:
        """
        Derive complete N_eff formula from FIRM morphic channel theory.
        ...
        derivation_steps.append("FIRM N_eff Derivation")
        derivation_steps.append("=====================")
        ...
        neff_firm = total_main + delta_reheat

        derivation_steps.append(f"\nStep 3: Add reheating correction")
        derivation_steps.append(f"N_eff = {total_main:.3f} + {delta_reheat:.3f} = {neff_firm:.3f}")

        # Step 4: Observational comparison
        neff_observed = 3.046
        relative_error = abs(neff_firm - neff_observed) / neff_observed * 100

        derivation_steps.append(f"\nStep 4: Observational comparison")
        derivation_steps.append(f"N_eff observed (Planck 2018) = {neff_observed:.3f}")
        derivation_steps.append(f"N_eff FIRM = {neff_firm:.3f}")
...
def main():
    ...
    print("FIRM Effective Neutrino Species: Morphic Channel Multiplicities")
    print("="*70)
    ...
    print(f"\nFinal Result:")
    print(f"N_eff FIRM = {result.theoretical_value:.3f}")
